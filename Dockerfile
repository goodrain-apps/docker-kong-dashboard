...
RUN git clone https://github.com/PGBI/kong-dashboard.git \
    && cd kong-dashboard \
    # chinaland begin
#    && curl --progress --remote-name https://github.com/sass/node-sass/releases/download/v3.13.0/linux-x64-48_binding.node \
#    && echo "sass_binary_path = /app/kong-dashboard/linux-x64-48_binding.node" > ~/.npmrc \
